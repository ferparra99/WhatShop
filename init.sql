-- ============================================================
--  WhatShop Marketplace — Script de Base de Datos
--  Generado desde entidades JPA reales del proyecto
--  Package base: com.whatshop.marketplace
--  Compatible con: PostgreSQL 15+
--  Última actualización: 2026-06-23
-- ============================================================
-- NOTA PARA IA: Este script refleja exactamente las entidades
-- JPA del proyecto. Cualquier cambio en las entidades debe
-- reflejarse aquí también.
-- ============================================================

-- Limpiar si ya existen (orden inverso de dependencias)
...
DO $$
BEGIN
    RAISE NOTICE '✅ Tabla users      creada: % filas', (SELECT COUNT(*) FROM users);
    RAISE NOTICE '✅ Tabla sellers     creada: % filas', (SELECT COUNT(*) FROM sellers);
    RAISE NOTICE '✅ Tabla categories  creada: % filas', (SELECT COUNT(*) FROM categories);
    RAISE NOTICE '✅ Tabla products    creada: % filas', (SELECT COUNT(*) FROM products);
    RAISE NOTICE '🚀 Base de datos WhatShop lista para desarrollo';
END $$;
